...
test("direct execution detection uses a platform-correct file URL", () => {
  assert.equal(isDirectExecution(import.meta.url, new URL(import.meta.url).pathname), true);
  assert.equal(isDirectExecution(import.meta.url, path.join(process.cwd(), "different.mjs")), false);
});
